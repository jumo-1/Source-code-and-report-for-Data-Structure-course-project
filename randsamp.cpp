...
Status GridInit(Mat& grid, double w, int& samples_number, Active* actives, int r)
{
	...
	data[cell_pos[0] * grid.channels() + 1] = original.y;			   //data的作用？
	data[cell_pos[0] * grid.channels() + 2] = 0;
	samples_number += 1;				    //初始值为0？
	ActiveAppend(actives, original, 0);					 //将初始点加入链表
...
{
	if (p->next == NULL)				//该情况为链表未初始化
		return ERROR;
	Active* temp = p->next;

	while (temp->next != NULL && temp->active_index != sample_index)	 //找目标节点
	{
		p = temp;
		temp = temp->next;
	}

	if (temp->active_index == sample_index)						 //删除目标节点
	{
		p->next = temp->next;
		free(temp);
		temp = NULL;
...
Status PoissonDisk(Mat& maskImage, int& count)
{
	...
	if (!(g_srcImage.type() == g_maskImage.type()) || !(g_srcImage.size() == g_maskImage.size())) {
		if (!(g_srcImage.type() == g_maskImage.type()) || !(g_srcImage.size() == g_maskImage.size())) {
			cout << "PoissonDisk调用错误——实参之间size不匹配！！" << endl;
			exit(0);
		}
		cout << "PoissonDisk调用错误——实参之间类型不匹配！！" << endl;
		exit(0);
	}
